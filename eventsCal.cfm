...
<cfparam name="PageNum_Events" default="1">
<cfquery name="Events" datasource="FSA1">
SELECT *
FROM Events
WHERE  eventDate > #LSDATEFORMAT(Now())-14#
Order by eventDate 
</cfquery>
<cfset MaxRows_Events=10>
<cfset StartRow_Events=Min((PageNum_Events-1)*MaxRows_Events+1,Max(Events.RecordCount,1))>
<cfset EndRow_Events=Min(StartRow_Events+MaxRows_Events-1,Events.RecordCount)>
<cfset TotalPages_Events=Ceiling(Events.RecordCount/MaxRows_Events)>
<cfset QueryString_Events=Iif(CGI.QUERY_STRING NEQ "",DE("&"&XMLFormat(CGI.QUERY_STRING)),DE(""))>
<cfset tempPos=ListContainsNoCase(QueryString_Events,"PageNum_Events=","&")>
<cfif tempPos NEQ 0>
  <cfset QueryString_Events=ListDeleteAt(QueryString_Events,tempPos,"&")>
</cfif>
<cfscript>
...
<title>Flathead Snowmobile Association</title>
...
<div id="MMMenuContainer0228062045_1">
    ...
    <h1>Snowmobiling Events</h1>
    <table border="0" align="center">
      <cfoutput>
        <tr>
          <td class="navText"><cfif PageNum_Events GT 1>
              <a href="#CurrentPage#?PageNum_Events=1#QueryString_Events#"><img src="WA_DataAssist/images/Pacifica/Refined_first.gif" border="0" />First</a>
            </cfif>
          </td>
          <td class="navText"><cfif PageNum_Events GT 1>
              <a href="#CurrentPage#?PageNum_Events=#Max(DecrementValue(PageNum_Events),1)##QueryString_Events#"><img src="WA_DataAssist/images/Pacifica/Refined_previous.gif" border="0" />Previous</a>
            </cfif>
          </td>
          <td class="navText"><cfif PageNum_Events LT TotalPages_Events>
              <a href="#CurrentPage#?PageNum_Events=#Min(IncrementValue(PageNum_Events),TotalPages_Events)##QueryString_Events#">Next<img src="WA_DataAssist/images/Pacifica/Refined_next.gif" border="0" /></a>
            </cfif>
          </td>
          <td class="navText"><cfif PageNum_Events LT TotalPages_Events>
              <a href="#CurrentPage#?PageNum_Events=#TotalPages_Events##QueryString_Events#">Last<img src="WA_DataAssist/images/Pacifica/Refined_last.gif" border="0" /></a>
            </cfif>
          </td>
        </tr>
      </cfoutput>
    ...
    <table border="0" cellspacing="0" class="WADADataTable">
      ...
      <tr>
        <td class="TableHdrHoriz"><div align="center">Event</div></td>
        <td class="TableHdrHoriz"><div align="center">Date</div></td>
        <td class="TableHdrHoriz"><div align="center">Time</div></td>
        <td class="TableHdrHoriz"><div align="center">Location</div></td>
        <td class="TableHdrHoriz"><div align="center">Details</div></td>
      </tr>
      <cfoutput query="Events" startRow="#StartRow_Events#" maxRows="#MaxRows_Events#">
        <tr class="#WA_AltClassIterator_getClass(WARRT_AltClass1,true)#">
          <td class="WADADataTableCell"><div align="center" class="style1">#Events.eventName#</div></td>
          <td class="WADADataTableCell"><div align="center">#Dateformat(Events.eventDate, "MM/DD/YY")#</div></td>
          <td class="WADADataTableCell"><div align="center">#Events.eventTime#</div></td>
          <td class="WADADataTableCell"><div align="center">#Events.eventLocation#</div></td>
          <td class="WADADataTableCell">#Events.eventNote#</td>
        </tr>
      </cfoutput>
      <tr class="tableBtmBar">
        <td colspan="5" class="tableBtmTxt"><img name="imgPlacehldr" src="" width="1" height="14" alt="" />
