...
SELECT * 
FROM tblAds 
WHERE AdsID = <cfqueryparam value="#URL.AdsID#" cfsqltype="cf_sql_numeric"> 
</cfquery>
<!--- Define the path to the image files folder --->
<cfset photoPath=ExpandPath("../images/")>
<!--- If submitting for update. --->
<cfif (isDefined("Form.Update.x")) >
  <!--- Capture the image filename --->
  <cfset Filename_Photo1 = "#WADAtblAds.AdsBanner#">
  <!--- Upload the new file and resize it --->
  <!--- If an AdsBanner image was selected for upload--->
  <cfif IsDefined("FORM.AdsBanner") AND #FORM.AdsBanner# NEQ "">
    <!--- Define the path to the exisiting image file. Be careful - case sensitive --->
    <cfset imageFilePath = Replace(ExpandPath("1"),"AdminWk\1", "images\#WADAtblAds.AdsBanner#" )>
    <!--- Delete the existing file if there is one linked in the table --->
    <cfif WADAtblAds.AdsBanner NEQ "">
      <cffile action="delete" file="#imageFilePath#">
    </cfif>
    <cfset FORM.AdsBanner = Replace("#FORM.AdsBanner#"," ","_","ALL")>
    <!--- Upload the image to the images folder --->
    <cffile  action="upload" destination="#photoPath#" filefield="AdsBanner" nameconflict="makeunique" accept="image/*">
    <!--- Replace the blanks in the filename with underscores  --->
    <cfset Replace("#CFFILE.ServerFile#"," ","_","ALL")>
    <!--- Assign the uploaded filename to a variable--->
    <cfset Filename_Photo1 = "#CFFILE.ServerFile#">
    <!--- Check the image orientation --->
    <cfimage source="#photoPath##CFFILE.ServerFile#" name="newImage">
    <cfset imgHeight = #ImageGetHeight(newImage)#>
    <cfset imgWidth = #ImageGetWidth(newImage)#>
    <!--- Resize the image --->
    <cfif #imgWidth# GT #imgHeight#>
      <cfimage source="#photoPath##CFFILE.ServerFile#" action="resize" height="120" Width="" Destination="#photoPath##Filename_Photo1#" overwrite="Yes">
      <cfelse>
      <cfimage source="#photoPath##CFFILE.ServerFile#" action="resize" height="120" Width="" Destination="#photoPath##Filename_Photo1#" overwrite="Yes">
    </cfif>
  </cfif>
  <!--- Update the record in the Ads table, write the Banner image filename --->
  <cfquery datasource="#APPLICATION.datasource#" name="updateImg">
	UPDATE tblAds 
    SET AdsBanner = '#Filename_Photo1#',
    	AdsBody = '#FORM.AdsBody#',
        AdsCustomerName = '#FORM.AdsCustomerName#',
        AdsLink = '#FORM.AdsLink#',
        AdsHeadline = '#FORM.AdsHeadline#',
        AdsStartDate = '#FORM.AdsStartDate#',
        AdsEndDate = '#FORM.AdsEndDate#',
        AdsAddress = '#FORM.AdsAddress#', 
        AdsCity = '#FORM.AdsCity#', 
        AdsState = '#FORM.AdsState#', 
        AdsZip = '#FORM.AdsZip#', 
        AdsPhone = '#FORM.AdsPhone#', 
        AdsEmail = '#FORM.AdsEmail#'
	WHERE AdsID = #URL.AdsID#
	</cfquery>
  <cflocation url="tblAds_Detail.cfm?AdsID=#FORM.AdsID#" addtoken="no">
</cfif>
        ...
        <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <div class="WADAUpdateContainer">
              <cfif WADAtblAds.RecordCount GT 0>
                <cfform action="tblAds_Update.cfm?AdsID=#URLEncodedFormat(WADAtblAds.AdsID)#" enctype="multipart/form-data" method="post" name="WADAUpdateForm" id="WADAUpdateForm">
                  <div class="WADAHeader"></div>
                  <div class="WADAHorizLine"><img src="../WA_DataAssist/images/_tx_.gif" alt="" height="1" width="1" border="0" /></div>
                  <table class="WADADataTable" cellpadding="0" cellspacing="0" border="0">
                    <tr>
                      <th class="WADADataTableHeader">Advertiser Name:</th>
                      <td class="WADADataTableCellNoPad"><table border="0" cellspacing="0" cellpadding="0">
                          <tr class="WADADataTable">
                            <td class="WADADataTableCell"><input type="text" name="AdsCustomerName" id="AdsCustomerName" value="<cfoutput>#Replace(WADAtblAds.AdsCustomerName, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
                            <th class="WADADataTableHeader">Price Paid:</th>
                            <td class="WADADataTableCell"><cfinput type="text" name="AdsPricePaid" id="AdsPricePaid" value="#WADAtblAds.AdsPricePaid#" size="10" /></td>
                          </tr>
                        </table></td>
                    </tr>
                    <tr>
                      <th class="WADADataTableHeader">Advertiser Address:</th>
                      <td class="WADADataTableCell"><input type="text" name="AdsAddress" id="AdsAddress" value="<cfoutput>#Replace(WADAtblAds.AdsAddress, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
                    </tr>
                    <tr>
                      <th class="WADADataTableHeader">City</th>
                      <td class="WADADataTableCell"><input type="text" name="AdsCity" id="AdsCity" value="<cfoutput>#Replace(WADAtblAds.AdsCity, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
                    </tr>
                    <tr>
                      <th class="WADADataTableHeader">State:</th>
                      <td class="WADADataTableCellNoPad"><table border="0" align="left" cellpadding="0" cellspacing="0">
                          <tr class="WADADataTable">
                            <td class="WADADataTableCell"><input type="text" name="AdsState" id="AdsState" value="<cfoutput>#Replace(WADAtblAds.AdsState, '"' , '&quot;', 'all')#</cfoutput>" size="6" /></td>
                            <td class="WADADataTableCell"><div align="right"><strong>Zip:</strong></div></td>
                            <td class="WADADataTableCell"><input type="text" name="AdsZip" id="AdsZip" value="<cfoutput>#Replace(WADAtblAds.AdsZip, '"' , '&quot;', 'all')#</cfoutput>" size="10" /></td>
                          </tr>
                        </table></td>
                    </tr>
                    <tr>
                      <th class="WADADataTableHeader">Ad Headline:</th>
                      <td class="WADADataTableCell"><input type="text" name="AdsHeadline" id="AdsHeadline" value="<cfoutput>#Replace(WADAtblAds.AdsHeadline, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
                    </tr>
                    <tr>
                      <th class="WADADataTableHeader">Ad Body Text:</th>
                      <td class="WADADataTableCell"><cftextarea name="AdsBody" cols="70" rows="3"><cfoutput>#Replace(WADAtblAds.AdsBody, '"' , '&quot;', 'all')#</cfoutput></cftextarea>
                      </td>
                    </tr>
                    <tr>
                      <th class="WADADataTableHeader">Ad Banner Image:</th>
                      <td class="WADADataTableCell"><cfinput type="file" name="AdsBanner" id="AdsBanner" value="" size="32"/>
                        <br/>
                        <span class="copyrightL">Current Banner Image</span><img src="../images/<cfoutput>#WADAtblAds.AdsBanner#</cfoutput>" width="120"  /></td>
                    </tr>
                    <tr>
                      <th class="WADADataTableHeader">Contact Email:</th>
                      <td class="WADADataTableCell"><input type="text" name="AdsEmail" id="AdsEmail" value="<cfoutput>#Replace(WADAtblAds.AdsEmail, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
                    </tr>
                    <tr>
                      <th class="WADADataTableHeader">Ad Web Link:</th>
                      <td class="WADADataTableCell"><input type="text" name="AdsLink" id="AdsLink" value="<cfoutput>#WADAtblAds.AdsLink#</cfoutput>" size="32" /></td>
                    </tr>
                    <tr>
                      <th class="WADADataTableHeader"> Contact Telephone:</th>
                      <td class="WADADataTableCell"><input type="text" name="AdsPhone" id="AdsPhone" value="<cfoutput>#Replace(WADAtblAds.AdsPhone, '"' , '&quot;', 'all')#</cfoutput>" size="32" />
                        <br />
                        888-555-1212</td>
                    </tr>
                    <tr>
                      <th class="WADADataTableHeader">Start Date:</th>
                      <td class="WADADataTableCell"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                          <tr>
                            <td class="WADADataTableCellNoPad"><cfinput type="datefield" name="AdsStartDate" id="AdsStartDate" value="#LSDateFormat(WADAtblAds.AdsStartDate)#"  required="yes" message="Start date cannot be blank." />
                              <br/>
                              <br/>
                              <br/></td>
                            <th class="WADADataTableHeader">End Date:</th>
                            <td class="WADADataTableCellNoPad"><cfinput type="datefield" name="AdsEndDate" id="AdsEndDate" value="#LSDateFormat(WADAtblAds.AdsEndDate)#"  required="yes" message="End date cannot be blank." />
                              <br/>
                              <br/>
                              <br/></td>
                          </tr>
                  ...
                  <div class="WADAButtonRow">
                    <table border="0" align="center" cellpadding="0" cellspacing="0" class="WADADataNavButtons">
                      <tr>
                        ...
                        <td class="WADADataNavButtonCell" nowrap="nowrap"><a href="tblAds_Results.cfm" title="Cancel"><img border="0" name="Cancel" id="Cancel" alt="Cancel" src="../WA_DataAssist/images/Pacifica/Refined_cancel.gif" /></a></td>
                      </tr>
                    </table>
                    <input name="AdsID" type="hidden" id="AdsID" value="<cfoutput>#URLEncodedFormat(WADAtblAds.AdsID)#</cfoutput>" />
                  </div>
                </cfform>
              </cfif>
