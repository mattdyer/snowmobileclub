...
<cfif ( Not WA_Auth_RulePasses("Logged in to Users") ) >
  <cfset WA_Auth_RestrictAccess("Users_LogIn.cfm") >
</cfif>
<cfparam name="SESSION.ID" default="-1">
<cfset CurrentPage=GetFileFromPath(GetBaseTemplatePath())>
<cfif IsDefined("FORM.MM_UpdateRecord") AND FORM.MM_UpdateRecord EQ "WAATKUpdateForm">
...
<cfquery name="WAATKUsers" datasource="FSA1">
...
<!-- #EndLibraryItem --><!--========================= STOP COPYING THE HTML HERE =========================-->
  ...
  <div id="mainContent"> <!-- InstanceBeginEditable name="Heading1" --><h1>Profile</h1><!-- InstanceEndEditable -->
      <div id="MainImg">
        <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <div id="UpdateContainer" class="WAATK">
            <cfif WAATKUsers.RecordCount GT 0>
              <form action="<cfoutput>#CurrentPage#</cfoutput>" method="post" name="WAATKUpdateForm" id="WAATKUpdateForm">
                <h1></h1>
                <table class="WAATKDataTable" cellpadding="0" cellspacing="0" border="0">
                  <tr>
                    <th>First Name:</th>
                    <td><input type="text" class="WAATKTextField" name="firstName" id="firstName" value="<cfoutput>#Replace(WAATKUsers.firstName, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
                  </tr>
                  <tr>
                    <th>Last Name:</th>
                    <td><input type="text" class="WAATKTextField" name="lastName" id="lastName" value="<cfoutput>#Replace(WAATKUsers.lastName, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
                  </tr>                  
                  <tr>
                    <th>UserName:</th>
                    <td><input type="text" class="WAATKTextField" name="userName" id="userName" value="<cfoutput>#Replace(WAATKUsers.userName, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
                  </tr>
                  <tr>
                    <th>Password:</th>
                    <td><input type="password" class="WAATKTextField" name="pword" id="pword" value="<cfoutput>#Replace(WAATKUsers.pword, '"' , '&quot;', 'all')#</cfoutput>" /></td>
                  </tr>
<!---                   <tr>
                    <th>Role:</th>
                    <td><input type="text" class="WAATKTextField" name="role" id="role" value="<cfoutput>#Replace(WAATKUsers.role, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
                  </tr> --->

                  <tr>
                    <th>Email Address:</th>
                    <td><input type="text" class="WAATKTextField" name="emailAddress" id="emailAddress" value="<cfoutput>#Replace(WAATKUsers.emailAddress, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
                  </tr>
                </table>
                <div class="WAATKButtonRow">
                  ...
                  <input name="ID" type="hidden" id="WAATKUpdateRecordID" value="<cfoutput>#URLEncodedFormat(WAATKUsers.ID)#</cfoutput>" />
                  <input type="hidden" name="MM_UpdateRecord" value="WAATKUpdateForm" />
                </div>
              </form>
            </cfif>
            <cfif WAATKUsers.RecordCount EQ 0>
