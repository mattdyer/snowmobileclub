...
SELECT ID, eventName, eventDate, eventTime, eventLocation, eventNote FROM Events WHERE ID = <cfqueryparam value="#URL.ID#" cfsqltype="cf_sql_numeric"> 
</cfquery>
<cfif (isDefined("Form.Update.x")) >
  <cfset WA_connection = "FSA1" />
  <cfset WA_table = "Events" />
  <cfset WA_redirectURL = "Events_Results.cfm" />
  <cfset WA_keepQueryString = false >
  <cfset WA_indexField = "ID" />
  <cfset WA_fieldNamesStr = "eventName|eventDate|eventTime|eventLocation|eventNote" />
  <cfset WA_fieldValuesStr = "" & ToString(IIf(isDefined("Form.eventName"),"Form.eventName",DE(""))) & "" & "|" & "" & ToString(IIf(isDefined("Form.eventDate"),"Form.eventDate",DE(""))) & "" & "|" & "" & ToString(IIf(isDefined("Form.eventTime"),"Form.eventTime",DE(""))) & "" & "|" & "" & ToString(IIf(isDefined("Form.eventLocation"),"Form.eventLocation",DE(""))) & "" & "|" & "" & ToString(IIf(isDefined("Form.eventNote"),"Form.eventNote",DE(""))) & "" />
  <cfset WA_columnTypesStr = "',none,''|##,none,NULL|',none,''|',none,''|',none,''" />
  <cfset WA_comparisonStr = " LIKE | = | LIKE | LIKE | LIKE " />
  ...
  <cfset WA_where_fieldValuesStr = "" & ToString(IIf(isDefined("Form.WADAUpdateRecordID"),"Form.WADAUpdateRecordID",DE(""))) & "" />
  <cfset WA_where_columnTypesStr = "none,none,NULL" />
  <cfset WA_where_comparisonStr = "=" />
  <cfset WA_where_fieldNames = WA_ListToArray(WA_indexField,"|") />
  <cfset WA_where_fieldValues = WA_ListToArray(WA_where_fieldValuesStr,"|") />
  <cfset WA_where_columns = WA_ListToArray(WA_where_columnTypesStr,"|") />
  <cfset WA_where_comparisions = WA_ListtoArray(WA_where_comparisonStr, "|") />
  ...
  <cfset WhereObj = WA_AB_generateWhereClause(WA_where_fieldNames, WA_where_columns, WA_where_fieldValues, WA_comparisions) />
  <cfquery datasource="#WA_connection#">
    UPDATE #WA_table#  SET #PreserveSingleQuotes(updateParamsObj3)# WHERE #PreserveSingleQuotes(WhereObj)#
  </cfquery>
...
<!-- #EndLibraryItem --><!--========================= STOP COPYING THE HTML HERE =========================-->
  ...
  <div id="mainContent"> <!-- InstanceBeginEditable name="Heading1" --><h1>Update Events</h1><!-- InstanceEndEditable -->
      <div id="MainImg">
        <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <div class="WADAUpdateContainer">
            <cfif WADAEvents.RecordCount GT 0>
              <cfform action="Events_Update.cfm?ID=#URLEncodedFormat(WADAEvents.ID)#" method="post" name="WADAUpdateForm" id="WADAUpdateForm">
                <div class="WADAHeader"></div>
                <div class="WADAHorizLine"><img src="../WA_DataAssist/images/_tx_.gif" alt="" height="1" width="1" border="0" /></div>
                <table class="WADADataTable" cellpadding="0" cellspacing="0" border="0">
                  <tr>
                    <th class="WADADataTableHeader">Event Name:</th>
                    <td class="WADADataTableCell"><cfinput type="text" name="eventName" id="eventName" value="#Replace(WADAEvents.eventName, '"' , '&quot;', 'all')#" size="32" /></td>
                  </tr>
                  <tr>
                    <th class="WADADataTableHeader">Date:</th>
                    <td class="WADADataTableCell"><cfinput type="datefield" name="eventDate" id="eventDate" value="#Dateformat(WADAEvents.eventDate,"MM/DD/YYYY")#" size="10"/></td>
                  </tr>
                  <tr>
                    <th class="WADADataTableHeader">Time:</th>
                    <td class="WADADataTableCell"><cfinput type="text" name="eventTime" id="eventTime" value="#Replace(WADAEvents.eventTime, '"' , '&quot;', 'all')#" size="32" /></td>
                  </tr>
                  <tr>
                    <th class="WADADataTableHeader">Location:</th>
                    <td class="WADADataTableCell"><cfinput type="text" name="eventLocation" id="eventLocation" value="#Replace(WADAEvents.eventLocation, '"' , '&quot;', 'all')#" size="32" /></td>
                  </tr>
                  <tr>
                    <th class="WADADataTableHeader">Details:</th>
                    <td class="WADADataTableCell"><!--- <input type="textarea" name="eventNote" id="eventNote" value="<cfoutput></cfoutput>" size="32" /> --->
                      
                      <cftextarea name="eventNote" cols="32" rows="4" id="eventNote"><cfoutput>#Replace(WADAEvents.eventNote, '"' , '&quot;', 'all')#</cfoutput></cftextarea>
</td>
                  </tr>
                </table>
                <div class="WADAHorizLine"><img src="../WA_DataAssist/images/_tx_.gif" alt="" height="1" width="1" border="0" /></div>
                <div class="WADAButtonRow">
                  <table class="WADADataNavButtons" border="0" cellpadding="0" cellspacing="0">
                    <tr>
                      ...
                      <td class="WADADataNavButtonCell" nowrap="nowrap"><a href="Events_Results.cfm" title="Cancel"><img border="0" name="Cancel" id="Cancel" alt="Cancel" src="../WA_DataAssist/images/Pacifica/Refined_cancel.gif" /></a></td>
                    </tr>
                  </table>
                  <input name="WADAUpdateRecordID" type="hidden" id="WADAUpdateRecordID" value="<cfoutput>#URLEncodedFormat(WADAEvents.ID)#</cfoutput>" />
                </div>
              </cfform>
            </cfif>
