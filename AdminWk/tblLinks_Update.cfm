...
<cfparam name="URL.linkID" default="-1">
<cfquery name="WADAtblLinks" datasource="FSA1">
SELECT linkID, topic, linkURL, linkDesc, lastDate, expDate FROM tblLinks WHERE linkID = <cfqueryparam value="#URL.linkID#" cfsqltype="cf_sql_numeric"> 
</cfquery>
<cfif (isDefined("Form.Update.x")) >
  <cfset WA_connection = "FSA1" />
  <cfset WA_table = "tblLinks" />
  <cfset WA_redirectURL = "tblLinks_Detail.cfm?linkID=" & ToString(IIf(isDefined("Form.WADAUpdateRecordID"),"Form.WADAUpdateRecordID",DE(""))) & "" />
  <cfset WA_keepQueryString = false >
  <cfset WA_indexField = "linkID" />
  <cfset WA_fieldNamesStr = "topic|linkURL|linkDesc|lastDate|expDate" />
  <cfset WA_fieldValuesStr = "" & ToString(IIf(isDefined("Form.topic"),"Form.topic",DE(""))) & "" & "|" & "" & ToString(IIf(isDefined("Form.linkURL"),"Form.linkURL",DE(""))) & "" & "|" & "" & ToString(IIf(isDefined("Form.linkDesc"),"Form.linkDesc",DE(""))) & "" & "|" & "" & ToString(IIf(isDefined("Form.lastDate"),"Form.lastDate",DE(""))) & "" & "|" & "" & ToString(IIf(isDefined("Form.expDate"),"Form.expDate",DE(""))) & "" />
  <cfset WA_columnTypesStr = "',none,''|',none,''|',none,''|##,none,NULL|##,none,NULL" />
        ...
        <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <div class="WADAUpdateContainer">
              <cfif WADAtblLinks.RecordCount GT 0><form action="tblLinks_Update.cfm?linkID=<cfoutput>#URLEncodedFormat(WADAtblLinks.linkID)#</cfoutput>" method="post" name="WADAUpdateForm" id="WADAUpdateForm"><div class="WADAHeader"></div>
                  <div class="WADAHorizLine"><img src="../WA_DataAssist/images/_tx_.gif" alt="" height="1" width="1" border="0" /></div>
                  <table class="WADADataTable" cellpadding="0" cellspacing="0" border="0">
                    <tr>
                      <th class="WADADataTableHeader">Topic:</th>
                      <td class="WADADataTableCell"><input type="text" name="topic" id="topic" value="<cfoutput>#Replace(WADAtblLinks.topic, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
                    </tr>
                    <tr>
                      <th class="WADADataTableHeader">URL:</th>
                      <td class="WADADataTableCell"><input type="text" name="linkURL" id="linkURL" value="<cfoutput>#Replace(WADAtblLinks.linkURL, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
                    </tr>
                    <tr>
                      <th class="WADADataTableHeader">Desc:</th>
                      <td class="WADADataTableCell"><input type="text" name="linkDesc" id="linkDesc" value="<cfoutput>#Replace(WADAtblLinks.linkDesc, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
                    </tr>
                    <tr>
                      <th class="WADADataTableHeader">Expiration Date:</th>
                      <td class="WADADataTableCell"><input type="text" name="expDate" id="expDate" value="<cfoutput>#DateFormat(WADAtblLinks.expDate,"MM/DD/yyyy")#</cfoutput>" size="12" /><input type="hidden" name="lastDate" id="lastDate" value="<cfoutput>#DateFormat(Now(),"MM/DD/yyyy")#</cfoutput>" /></td>
                    </tr>
                  </table>
                  <div class="WADAHorizLine"><img src="../WA_DataAssist/images/_tx_.gif" alt="" height="1" width="1" border="0" /></div>
                  <div class="WADAButtonRow">
                    ...
                    <input name="WADAUpdateRecordID" type="hidden" id="WADAUpdateRecordID" value="<cfoutput>#URLEncodedFormat(WADAtblLinks.linkID)#</cfoutput>" />
                  </div>
                </form>
              </cfif>
