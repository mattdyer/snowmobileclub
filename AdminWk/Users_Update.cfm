...
SELECT ID, userName, pword, role, firstName, lastName, emailAddress FROM Users WHERE ID = <cfqueryparam value="#URL.ID#" cfsqltype="cf_sql_numeric"> 
</cfquery>
<cfif (isDefined("Form.Update.x")) >
  <cfset WA_connection = "FSA1" />
  <cfset WA_table = "Users" />
  <cfset WA_redirectURL = "Users_Results.cfm" />
  <cfset WA_keepQueryString = false >
  <cfset WA_indexField = "ID" />
  <cfset WA_fieldNamesStr = "userName|pword|role|firstName|lastName|emailAddress" />
  <cfset WA_fieldValuesStr = "" & ToString(IIf(isDefined("Form.userName"),"Form.userName",DE(""))) & "" & "|" & "" & ToString(IIf(isDefined("Form.pword"),"Form.pword",DE(""))) & "" & "|" & "" & ToString(IIf(isDefined("Form.role"),"Form.role",DE(""))) & "" & "|" & "" & ToString(IIf(isDefined("Form.firstName"),"Form.firstName",DE(""))) & "" & "|" & "" & ToString(IIf(isDefined("Form.lastName"),"Form.lastName",DE(""))) & "" & "|" & "" & ToString(IIf(isDefined("Form.emailAddress"),"Form.emailAddress",DE(""))) & "" />
  <cfset WA_columnTypesStr = "',none,''|',none,''|',none,''|',none,''|',none,''|',none,''" />
  <cfset WA_comparisonStr = " LIKE | LIKE | LIKE | LIKE | LIKE | LIKE " />
...
<!-- #EndLibraryItem --><!--========================= STOP COPYING THE HTML HERE =========================-->
  ...
  <div id="mainContent"> <!-- InstanceBeginEditable name="Heading1" -->
    ...
    <div class="WADAUpdateContainer">
      <cfif WADAUsers.RecordCount GT 0><form action="Users_Update.cfm?ID=<cfoutput>#URLEncodedFormat(WADAUsers.ID)#</cfoutput>" method="post" name="WADAUpdateForm" id="WADAUpdateForm"><div class="WADAHeader"></div>
          <div class="WADAHorizLine"><img src="../WA_DataAssist/images/_tx_.gif" alt="" height="1" width="1" border="0" /></div>
          <table class="WADADataTable" cellpadding="0" cellspacing="0" border="0">
            <tr>
              <th class="WADADataTableHeader">First Name:</th>
              <td class="WADADataTableCell"><input type="text" name="firstName" id="firstName" value="<cfoutput>#Replace(WADAUsers.firstName, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
            </tr>
            <tr>
              <th class="WADADataTableHeader">Last Name:</th>
              <td class="WADADataTableCell"><input type="text" name="lastName" id="lastName" value="<cfoutput>#Replace(WADAUsers.lastName, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
            </tr>            
            <tr>
              <th class="WADADataTableHeader">UserName:</th>
              <td class="WADADataTableCell"><input type="text" name="userName" id="userName" value="<cfoutput>#Replace(WADAUsers.userName, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
            </tr>
            <tr>
              <th class="WADADataTableHeader">Password:</th>
              <td class="WADADataTableCell"><input type="password" name="pword" id="pword" value="<cfoutput>#Replace(WADAUsers.pword, '"' , '&quot;', 'all')#</cfoutput>" /></td>
            </tr>
<!---             <tr>
              <th class="WADADataTableHeader">Role:</th>
              <td class="WADADataTableCell"><input type="radio" name="Role" value="user" checked >Regular User&nbsp;&nbsp;&nbsp;&nbsp;
						    <input type="radio" name="role" value="admin">Admin</div></td>
            </tr> --->

            <tr>
              <th class="WADADataTableHeader">Email Address:</th>
              <td class="WADADataTableCell"><input type="text" name="emailAddress" id="emailAddress" value="<cfoutput>#Replace(WADAUsers.emailAddress, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
            </tr>
          </table>
          <div class="WADAHorizLine"><img src="../WA_DataAssist/images/_tx_.gif" alt="" height="1" width="1" border="0" /></div>
          <div class="WADAButtonRow">
            <table class="WADADataNavButtons" border="0" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <td class="WADADataNavButtonCell" nowrap="nowrap"><a href="Users_Results.cfm" title="Cancel"><img border="0" name="Cancel" id="Cancel" alt="Cancel" src="../WA_DataAssist/images/Pacifica/Refined_cancel.gif" /></a></td>
              </tr>
            </table>
            <input name="WADAUpdateRecordID" type="hidden" id="WADAUpdateRecordID" value="<cfoutput>#URLEncodedFormat(WADAUsers.ID)#</cfoutput>" />
          </div>
        </form>
      </cfif>
