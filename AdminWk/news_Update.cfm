...
SELECT ID, newsDate, newsHeadline, newsDetails FROM news WHERE ID = <cfqueryparam value="#URL.ID#" cfsqltype="cf_sql_numeric"> 
</cfquery>
<cfif (isDefined("Form.Update.x")) >
  <cfset WA_connection = "FSA1" />
  <cfset WA_table = "news" />
  <cfset WA_redirectURL = "news_Detail.cfm?ID=" & ToString(IIf(isDefined("Form.WADAUpdateRecordID"),"Form.WADAUpdateRecordID",DE(""))) & "" />
  <cfset WA_keepQueryString = false >
  <cfset WA_indexField = "ID" />
  <cfset WA_fieldNamesStr = "newsDate|newsHeadline|newsDetails" />
  <cfset WA_fieldValuesStr = "" & ToString(IIf(isDefined("Form.newsDate"),"Form.newsDate",DE(""))) & "" & "|" & "" & ToString(IIf(isDefined("Form.newsHeadline"),"Form.newsHeadline",DE(""))) & "" & "|" & "" & ToString(IIf(isDefined("Form.newsDetails"),"Form.newsDetails",DE(""))) & "" />
  <cfset WA_columnTypesStr = "##,none,NULL|',none,''|',none,''" />
  <cfset WA_comparisonStr = " = | LIKE | LIKE " />
        ...
        <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <div class="WADAUpdateContainer">
              <cfif WADAnews.RecordCount GT 0><cfform action="news_Update.cfm?ID=#URLEncodedFormat(WADAnews.ID)#" method="post" name="WADAUpdateForm" id="WADAUpdateForm"><div class="WADAHeader"></div>
                  <div class="WADAHorizLine"><img src="../WA_DataAssist/images/_tx_.gif" alt="" height="1" width="1" border="0" /></div>
                  <table class="WADADataTable" cellpadding="0" cellspacing="0" border="0">
                    <tr>
                      <th class="WADADataTableHeader">Date:</th>
                      <td class="WADADataTableCell"><cfinput type="datefield" name="newsDate" id="newsDate" value="#DateFormat(WADAnews.newsDate,"MM/DD/YYYY")#"  required="yes" message="Date cannot be blank." /><br/><br/><br/>
					  </td>
                    </tr>
                    <tr>
                      <th class="WADADataTableHeader">Headline:</th>
                      <td class="WADADataTableCell"><input type="text" name="newsHeadline" id="newsHeadline" value="<cfoutput>#Replace(WADAnews.newsHeadline, '"' , '&quot;', 'all')#</cfoutput>" size="32" /></td>
                    </tr>
                    <tr>
                      <th class="WADADataTableHeader">Details:</th>
                      <td class="WADADataTableCell"><textarea name="newsDetails" cols="50" rows="10" id="newsDetails"><cfoutput>#Replace(WADAnews.newsDetails, '"' , '&quot;', 'all')#</cfoutput></textarea><!--- <input type="text" name="newsDetails" id="newsDetails" value="<cfoutput>#Replace(WADAnews.newsDetails, '"' , '&quot;', 'all')#</cfoutput>" size="32" /> ---></td>
                    </tr>
                  </table>
                  <div class="WADAHorizLine"><img src="../WA_DataAssist/images/_tx_.gif" alt="" height="1" width="1" border="0" /></div>
                  <div class="WADAButtonRow">
                    ...
                    <input name="WADAUpdateRecordID" type="hidden" id="WADAUpdateRecordID" value="<cfoutput>#URLEncodedFormat(WADAnews.ID)#</cfoutput>" />
                  </div>
                </cfform>
              </cfif>
